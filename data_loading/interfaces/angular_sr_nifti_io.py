...
class AngularSRNiftiIO(Abstract_IO):
    ...
    #---------------------------------------------#
    def load_downsampled_mif_dwi(self, path, index = None, grad_dirs = 6, needs_affine = False, random = True):
        ...
        if path.endswith('.mif.gz') or path.endswith('.mif'):
            ...
            lr_bvecs, lr_bvals, lr_index, b0_index = extract_single_shell(
                bvals, bvecs, directions = grad_dirs, extract_bval = 1000, sample=random
            )
