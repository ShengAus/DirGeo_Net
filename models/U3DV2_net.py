...
class SEModule(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_ratio= 16):
        super(SEModule, self).__init__()
        assert in_channels == out_channels
        self.conv = torch.nn.Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=out_channels)
        
        self.linear_layers = nn.Sequential(nn.Linear(out_channels, out_channels//hidden_ratio)
                                           ,nn.ReLU()
                                           ,nn.Linear(out_channels//hidden_ratio, out_channels)
                                           ,nn.Sigmoid())
    def forward(self,x):
        out =  self.linear_layers(self.conv(x).permute(0,2,3,4,1))
        return out.permute(0,4,1,2,3)

class SEConv(torch.nn.Module):
    """
    Gated Convlution layer with activation (default activation:LeakyReLU)
    """
    def __init__(self, in_channels, out_channels
        ...
        self.conv3d = torch.nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.batch_norm3d = torch.nn.BatchNorm3d(out_channels)
        self.semodule = SEModule(out_channels,out_channels)
        self.sigmoid = torch.nn.Sigmoid()
...
class U3DV2net(nn.Module):
    ...
    def forward(self, x, encoder_only=False):
        ...
        out = self.out2(out.permute(0,2,3,4,1)).permute(0,4,1,2,3)
