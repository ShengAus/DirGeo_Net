...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        for j in range(0, patch_nums, opt.input_batch_sizes):
            ...
            pred_dti_patch= model.test()  # run inference

            pred_dti_patch = pred_dti_patch.detach().cpu().numpy()

            # b c w h d -> b w h d c
            pred_dti_patch = np.transpose(pred_dti_patch, (0, 2, 3, 4, 1))
            pred_dti_patches.append(pred_dti_patch)


        # Postprocess predicted patches
        pred_dti_patches = np.concatenate(pred_dti_patches, axis=0)


        preds['pred_dti'] =  pred_dti_patches
        # preds['dwi_unnorm'] =  data['dwi_unnorm'].permute(0, 2, 3, 4, 1).numpy()
        preds['dwi'] = data['dwi'].permute(0, 2, 3, 4, 1).numpy()

        predictions_dict = dataset.dataset.postprocessing(preds)
        resulted_img = predictions_dict['pred_dti']
        resulted_fa, resulted_md, resulted_ad = get_dti_metrics(predictions_dict['pred_dti'])
        ...
        if (i+1) % opt.save_prediction == 0:
            # dataset.dataset.save_prediction(resulted_fa, suffix = '_dti_wm_pred')
            ...
            dataset.dataset.save_prediction(predictions_dict['pred_dti'], suffix = '_dti_pred')
            # dataset.dataset.save_prediction(predictions_dict['dwi_unnorm'], suffix = '_dwi_unnorm')
            # dataset.dataset.save_prediction(predictions_dict['dwi'], suffix = '_dwi')
            # dataset.dataset.save_prediction(data['gt_dti'], suffix = '_gt_dti')
