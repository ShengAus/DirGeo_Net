...
def kenStone(X, k, random=True, metric='euclidean'):
    # Number of samples
    n = len(X)
    # print(n)
...
def extract_single_shell(bvals, bvecs, extract_bval=1000, extract_range=20, directions=32, sample=True):
    ...
    if sample:
        lr_index = kenStone(bvecs_input, directions, random = True)
        ...
        lr_index = kenStone(bvecs_input, directions, random = False)
